...
\- Non-privileged administrative user (`mrasavong`) with `wheel` group membership for `sudo`
...
\- SSH server configuration (`/etc/ssh/sshd\_config`) changes:
...
\- `firewalld`'s service-based model (`services: ssh`) is easier to reason about and safer than raw port numbers; using named services is a small but real defence against misconfiguration.
...
\- (further directories and files will be added as phases progress)
